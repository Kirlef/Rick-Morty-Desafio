...
class login : Fragment() {
    ...
    fun user_login(view:View){
        ...
        if (email.isEmpty())
        {
            view.findViewById<EditText>(R.id.login_email).error="Please insert a registered email"
            return
        }
        ...
        mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(){task->
            if(task.isSuccessful) {
                ...
                Toast.makeText(activity, "Please check your email or password", Toast.LENGTH_SHORT).show()
            }
        }
